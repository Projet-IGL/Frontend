...
/**
 * Garde de route pour protéger l'accès aux pages réservées aux utilisateurs ayant le rôle "Radiologue".
 * 
 * Cette garde empêche l'accès à la route si l'utilisateur n'est pas authentifié ou si son rôle 
 * n'est pas "Radiologue". En cas d'accès refusé, l'utilisateur est redirigé vers la page de connexion.
 * 
 ...
 * 
 * @returns {boolean} - Retourne `true` si l'utilisateur est authentifié et a le rôle "Radiologue", sinon `false`.
 */
export const radiologueGuard: CanActivateFn = (route, state) => {
  ...
  if (user && user.role === 'Radiologue') {
    return true; // L'accès est autorisé si l'utilisateur a le rôle "Radiologue"
  } else {
